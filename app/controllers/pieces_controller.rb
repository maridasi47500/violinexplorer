class PiecesController < ApplicationController
  ...
  def index
    @pieces = Piece.all.mylength(params[:length],params[:lengthasc]).mydate(params[:date],params[:dateasc]).tempo(params[:tempo],params[:tempoasc])
    if params[:tempo]||params[:length]||params[:date]
      @playlist=Playlist.new
      @playlist.pieces << @pieces
    end
  end
  ...
  private
    ...
    def piece_params
      params.require(:piece).permit(:tempo, :date, :title, :composer, :length, :style)
    end
end
